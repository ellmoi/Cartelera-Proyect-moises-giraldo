class MovieCard extends HTMLElement {
    ...
    render() {
        ...
        this.innerHTML = `<article class="movie-card"><button class="movie-card__visual" type="button" data-movie-id="${movie.id}" aria-label="Abrir ${title}">${poster}<span class="movie-card__overlay" aria-hidden="true"><span class="movie-card__overlay-title">${title}</span><span class="movie-card__genres">${genres}</span><span class="movie-card__rating"><span aria-hidden="true">★</span> ${rating}</span></span></button><h3>${title}</h3></article>`;
