...
function transferPaymentFields(values,errors){
    ...
    return `<div class="payment-method-panel"><label for="paymentBank">Banco</label><select id="paymentBank" name="bank"><option value="">Selecciona un banco</option>${banks.map(bank=>`<option value="${bank}" ${values.bank===bank?"selected":""}>${bank}</option>`).join("")}</select>${paymentError("bank",errors)}<label for="transferHolder">Nombre del titular</label><input id="transferHolder" name="holderName" value="${values.holderName||""}" autocomplete="name">${paymentError("holderName",errors)}<label for="holderDocument">Documento del titular</label><input id="holderDocument" name="holderDocument" inputmode="numeric" value="${values.holderDocument||""}">${paymentError("holderDocument",errors)}<label for="transferReference">Número o referencia de transferencia</label><input id="transferReference" name="transferReference" value="${values.transferReference||""}">${paymentError("transferReference",errors)}<p class="customer-form__help">Nunca solicitamos contraseñas, claves bancarias ni códigos OTP.</p></div>`;
}
function cardPaymentFields(values,errors){
    return `<div class="payment-method-panel"><label for="cardHolder">Nombre del titular</label><input id="cardHolder" name="holderName" autocomplete="cc-name" value="${values.holderName||""}">${paymentError("holderName",errors)}<label for="cardNumber">Número de tarjeta simulada</label><input id="cardNumber" name="cardNumber" inputmode="numeric" autocomplete="cc-number" placeholder="0000 0000 0000 0000">${paymentError("cardNumber",errors)}<div class="payment-card-row"><div><label for="cardExpiry">Fecha de vencimiento</label><input id="cardExpiry" name="expiry" inputmode="numeric" autocomplete="cc-exp" placeholder="MM/AA">${paymentError("expiry",errors)}</div><div><label for="cardCvv">CVV</label><input id="cardCvv" name="cvv" type="password" inputmode="numeric" autocomplete="off" maxlength="4">${paymentError("cvv",errors)}</div></div><p class="customer-form__help">La tarjeta es ficticia. El número completo, vencimiento y CVV no se guardan.</p></div>`;
}
function displayPaymentForm(context,errors,values){
    pendingPaymentContext=context;errors=errors||{};values=values||{};const method=values.paymentMethod||"";const isTransfer=method==="transfer",isCard=method==="card";
    moviesContainer.innerHTML=`<section class="reservation-view payment-view" aria-labelledby="paymentTitle"><p class="movie-details__eyebrow">THE MOI CINEMAS</p><h2 id="paymentTitle">Datos de pago</h2><p class="payment-academic-notice"><strong>Pago simulado — Proyecto académico</strong><br>No se procesará dinero real ni se contactará a bancos.</p><form class="customer-form payment-form" id="paymentForm" novalidate><fieldset><legend>Método de pago</legend><label class="payment-option"><input type="radio" name="paymentMethod" value="transfer" ${isTransfer?"checked":""}> Transferencia bancaria</label><label class="payment-option"><input type="radio" name="paymentMethod" value="card" ${isCard?"checked":""}> Tarjeta débito/crédito simulada</label>${paymentError("paymentMethod",errors)}</fieldset><label for="paymentPhone">Número de celular</label><input id="paymentPhone" name="phone" type="tel" inputmode="tel" autocomplete="tel" placeholder="+57 300 123 4567" value="${values.phone||""}"><span class="customer-form__help">No se enviará un SMS real.</span>${paymentError("phone",errors)}<div id="paymentMethodFields">${isTransfer?transferPaymentFields(values,errors):isCard?cardPaymentFields(values,errors):""}</div><div class="reservation-actions"><button class="secondary-action" id="backFromPayment" type="button">Volver</button><button class="primary-action" type="submit">Continuar al resumen</button></div></form></section>`;
}
...
function displayPaymentSummary(message){
    const summary=getPaymentSummaryContext(),methodDetail=safePaymentData.paymentMethod==="transfer"?safePaymentData.bank:"Terminación **** "+safePaymentData.cardLast4;
    moviesContainer.innerHTML=`<section class="reservation-view payment-summary" aria-labelledby="paymentSummaryTitle"><p class="movie-details__eyebrow">THE MOI CINEMAS</p><h2 id="paymentSummaryTitle">Resumen de compra</h2><p class="payment-academic-notice">Pago simulado — Proyecto académico</p>${message?`<p class="reservation-status" role="status">${message}</p>`:""}<dl class="reservation-details"><div><dt>Película</dt><dd>${summary.movieTitle}</dd></div><div><dt>Sala</dt><dd>${summary.room.name} · ${summary.room.type}</dd></div><div><dt>Fecha</dt><dd>${formatShowtimeDate(summary.movieFunction.date)}</dd></div><div><dt>Hora</dt><dd>${summary.movieFunction.time}</dd></div><div class="reservation-details__wide"><dt>Asientos</dt><dd class="selected-seats-details">${summary.seats.map(seat=>`${seat.seatCode} — Fila ${seat.row||seat.seatCode.charAt(0)} — Número ${seat.number||seat.seatCode.slice(1)} — ${seat.location}`).join("\n")}</dd></div><div><dt>Cantidad</dt><dd>${summary.quantity}</dd></div><div><dt>Precio unitario</dt><dd>${formatShowtimePrice(summary.unitPrice)}</dd></div><div><dt>Total</dt><dd>${formatShowtimePrice(summary.total)}</dd></div><div><dt>Método de pago</dt><dd>${paymentMethodLabel(safePaymentData.paymentMethod)} · ${methodDetail}</dd></div><div><dt>Usuario</dt><dd>${summary.userName}<br>${summary.email}</dd></div></dl><div class="reservation-actions"><button class="secondary-action" id="editPaymentData" type="button">Volver</button><button class="primary-action" id="confirmSimulatedPayment" type="button">Confirmar pago</button></div></section>`;
}
function handlePaymentSubmit(event){event.preventDefault();const values=collectPaymentValues(event.target),errors=validatePaymentValues(values);if(Object.keys(errors).length)return displayPaymentForm(pendingPaymentContext,errors,values);safePaymentData={paymentMethod:values.paymentMethod,phone:normalizeColombianPhone(values.phone),paymentStatus:"approved",paymentReference:generatePaymentReference()};if(values.paymentMethod==="transfer")safePaymentData.bank=values.bank;else safePaymentData.cardLast4=values.cardNumber.replace(/\D/g,"").slice(-4);displayPaymentSummary();}
...
moviesContainer.addEventListener("click",function(event){
    ...
    if(event.target.id==="editPaymentData"){displayPaymentForm(pendingPaymentContext,{},safePaymentData?{paymentMethod:safePaymentData.paymentMethod,phone:safePaymentData.phone,bank:safePaymentData.bank||""}:{});return;}
